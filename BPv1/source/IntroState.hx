	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('hyper_space_cut'), 0);
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menustuff/leTitleBG'));
		bg.updateHitbox();
		add(bg);
		bg.screenCenter();
		...
		clouds.frames = Paths.getSparrowAtlas('menustuff/clouds');
		clouds.animation.addByPrefix('idle', 'fluff', 11);
		clouds.animation.play('idle');
		mainGroup.add(clouds);
		clouds.screenCenter();		
		clouds.antialiasing = ClientPrefs.globalAntialiasing;

		var city:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menustuff/titleCity'));
		city.updateHitbox();
		mainGroup.add(city);
		city.antialiasing = ClientPrefs.globalAntialiasing;city.screenCenter();
		
		var bar:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menustuff/titleBar'));
		bar.updateHitbox();
		mainGroup.add(bar);
		...
		chars = new FlxSprite();
		chars.frames = Paths.getSparrowAtlas('menustuff/titleChars');
		chars.animation.addByPrefix('idle', 'bounce', 11);
		...
		var newLogo:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menustuff/logoNew'));
		newLogo.updateHitbox();
		mainGroup.add(newLogo);
